...
public class OwnerInfoDelegate3 extends LatteDelegate {

    @BindView(R2.id.text_shop_owner3_qq)
    TextView mQqText = null;
    @BindView(R2.id.text_shop_owner3_wx)
    TextView mWxText = null;
    @BindView(R2.id.text_shop_owner3_phone)
    TextView mPhoneText = null;

    @Override
    public Object setLayout() {
        return R.layout.delegate2_owner3_info;
    }

    @OnClick(R2.id.btn_shop_owner3_submit)
    void onClickSubmit() {
        ...
        query_name.findInBackground(new FindCallback<AVObject>() {
            ...
            public void done(List<AVObject> list, AVException e) {
                if (list.get(0).toJSONObject().toString() != null) {
                    ...
                    JSONArray ownerInfoList = JSON.parseObject(ownerInfoStr).getJSONArray("owner_phone3");
